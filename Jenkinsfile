            ...
            parallel {
                stage('Package Evaluation') {
                    steps {
                        ...
                        sh 'ls $(pwd)'
                        synopsys_detect '--detect.tools=DETECTOR --detect.project.name=InsecureBank --detect.project.version.name=Packages-1.0.${BUILD_NUMBER} --detect.project.codelocation.prefix=pom'
                    }
                }
